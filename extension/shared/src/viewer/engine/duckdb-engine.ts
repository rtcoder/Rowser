...
function buildImportSql(fileName: string, formatHint: SourceFormatHint): string {
  const options = formatHint === 'tsv' ? ", delim='\\t'" : '';

  return `CREATE TEMP TABLE ${TABLE_NAME} AS SELECT row_number() OVER () - 1 AS ${INTERNAL_ROW_ID_COLUMN}, * FROM read_csv_auto('${fileName}'${options})`;
}
